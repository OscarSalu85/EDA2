...
void save_character_data(Data* data){
    cJSON root = startup_write("SaveData.json");
    cJSON *sections = cJSON_GetObjectItem(&root, "Character");
    cJSON *section;
    cJSON_ArrayForEach(section,sections){
        cJSON_AddItemToObject("name", data->character->name,);
    }
}
